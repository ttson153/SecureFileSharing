        ...
        public Void doInBackground() {
            ...
            try {
                ...
                if (ret == JFileChooser.APPROVE_OPTION) {
                    ...
                    encryptedOutput.setText(String.format("Encrypted Output :  %s\t\n\n", fileName));
                    decryptedOutput.setText(String.format("Decrypted Output  : %s\t\n\n", fileName));
                    BufferedReader in = new BufferedReader(new FileReader(file));
                    String line = in.readLine();
                    long readLength = 0;
                    long totalLength = file.length();
                    double lengthPerPercent = 100.0 / totalLength;
                    // added dummy delay because encryption takes very less time in MVCE
                    Thread.sleep(1000);
                    while (line != null) {
                        try {
                            Thread.sleep(100);
